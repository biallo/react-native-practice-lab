export const lesson = {
  ...
  theme: '性能与工具',
  whyItMatters:
    '截至 2026-05-04，本课程覆盖到 React Native 0.85。0.83-0.85 的重点是把性能和工具链改进继续前移到默认路径：React 19.2、Hermes V1 默认、iOS 预编译二进制、DevTools 增强、新动画后端、Jest preset 独立。它们共同说明 RN 正在减少"每个项目自己调性能和工具"的成本，但也要求团队持续维护 Node、测试、构建和动画依赖的版本边界。\n\n这个阶段的特殊意义在于 RN 的发展方向变得越来越清晰：1) 核心包（react-native）的职责进一步聚焦，工具链分散到专门的包（jest-preset、devtools 等）；2) 性能默认值不断前移，新项目开箱即用的性能基线提高；3) 依赖的外部工具越来越多（Node、Xcode、JDK），项目团队需要维护更复杂的工具链；4) 调试和测试能力内置化，开发体验改进；5) 大版本升级变得更复杂，需要同时升级多个工具链组件。',
  features: [
    {
      title: 'React 19.2 与 DevTools',
      body: '0.83 更新到 React 19.2，并继续增强 RN DevTools。React 错误栈、调试能力和 renderer 行为会逐步与 React 最新能力对齐，但应用仍要关注 RN renderer 的支持范围。\n\nReact 19 的新特性：1) use() hook：更优雅的异步数据处理；2) Server Components：为后续离线和边界计算做准备；3) ref callback cleanup：更精确的资源管理；4) 改进的错误栈：更容易定位问题。但这些特性在 RN renderer 中的支持程度需要检查。'
    },
    {
      title: 'Hermes V1 by default',
      body: '0.84 将 Hermes V1 设为默认 JS 引擎，让编译器和 VM 改进成为新项目默认收益。对性能敏感项目，要重新建立启动、TTI、内存和长任务基线，而不是沿用旧 Hermes 结论。\n\nHermes V1 的改进包括：1) 更快的字节码编译；2) 改进的 JIT 编译（允许在某些情况下 JIT 优化）；3) 更激进的内存压缩和 GC；4) 更好的 source map 支持；5) 开发模式性能改进。但这也意味着某些原来在 JSC 或旧 Hermes 上的性能假设需要重新验证。'
    },
    {
      title: 'Precompiled iOS binaries',
      body: '0.84 默认使用 iOS 预编译二进制，减少首次构建和 native dependency 编译成本。这会改变 CI 时间、缓存策略和排查原生编译问题的方式。\n\n预编译二进制的含义：1) 第一次 pod install 会下载预构建的 .xcframework，而不是从源码编译；2) CI 时间大幅减少（从 10+ 分钟到 1 分钟）；3) 但这也意味着无法直接修改 RN 源码调试；4) 如果需要自定义 RN build，需要回退到从源码编译的模式；5) 二进制版本和源码版本需要保持一致。'
    },
    {
      title: 'New Animation Backend',
      body: '0.85 引入共享动画后端，让 Animated 和 Reanimated 底层能力更统一。Animated 可以用 native driver 驱动更多布局属性动画，减少 JS 线程参与带来的掉帧风险。\n\n新动画后端的能力：1) useNativeDriver 现在支持更多属性（如 width、height）；2) 动画帧率和主线程脱耦；3) 复杂手势动画（如 PanResponder + Animated）可以更顺滑；4) Reanimated 的性能改进；5) 但某些高阶用法仍需要监控性能。'
    },
    {
      title: 'Jest preset 独立',
      body: '0.85 将 Jest preset 从 react-native 包中拆到 @react-native/jest-preset。这个变化看似小，但体现出核心包边界收缩，测试工具链也需要显式维护。\n\n独立的原因和影响：1) 解耦 RN 版本和 Jest 版本迭代；2) Jest 可以独立快速迭代，不需要等 RN 大版本；3) 项目需要显式安装和更新 jest-preset；4) babel、ts-jest 等集成可能需要调整；5) 旧配置的 react-native preset 不再工作。'
    },
    {
      title: 'Node 与工具链最低线',
      body: '0.85 放弃 EOL Node 版本支持。RN 项目升级时，Node、Xcode、JDK、Gradle、CocoaPods、Jest 都可能成为前置条件。\n\n完整工具链的维护成本：1) Node：>=20；2) Xcode：>=15 (iOS)；3) JDK：>=17 (Android)；4) Gradle：>=8.0；5) CocoaPods：>=1.14；6) Jest：>=29；7) CLI 工具：>=0.85。团队需要有明确的升级计划和验证流程，而不是随意升级导致 CI 失败。'
    }
  ],
  deepDive: [
    {
