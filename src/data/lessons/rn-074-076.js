export const lesson = {
  ...
  theme: '新架构',
  whyItMatters:
    'New Architecture 是 RN 从旧 Bridge 时代走向 JSI、Fabric、TurboModules 和 Bridgeless 的系统性重构。0.76 默认启用意味着它不再只是高级选项，而是主路径。学习这一段要从"怎么打开开关"深入到"为什么架构要换"：渲染系统要支持 React 并发能力，原生模块调用要减少序列化和异步边界限制，三方库也要围绕新 contract 重新适配。\n\nNew Architecture 的核心动机包括：1) 旧 Bridge 是异步的，频繁跨边界调用会积累延迟，新 JSI 提供同步访问能力；2) 旧渲染流程中 JS 和 Native 是分离的，新 Fabric 统一调度，支持 React 并发；3) 原生模块的加载是"all or nothing"的，TurboModules 支持按需加载和tree-shaking；4) 整个生态需要一个清晰的现代化方向，不能永远维护两套系统。',
  features: [
    {
      title: 'Yoga 3.0',
      body: 'Yoga 是 RN 的跨平台布局引擎。Yoga 3.0 带来更现代的 Flexbox 行为和修正，也提醒我们：RN 布局不是浏览器 CSS 引擎，布局变化可能影响真实页面，需要视觉回归和关键页面验证。\n\nYoga 3.0 的关键变化：1) gap 支持改进，现在更接近 CSS Flexbox 规范；2) 百分比单位处理更精确；3) 某些边界情况的布局结果改变，这可能导致页面显示位置微调；4) 性能改进，布局计算更快。即使是"小改"也需要在 release 前全面验证。'
    },
    {
      title: 'Bridgeless New Architecture',
      body: 'Bridgeless 减少旧 Bridge 依赖，让 JS 与 Native 通过 JSI 等机制更直接互操作。这不是单个性能开关，而是模块系统、渲染系统、事件和调度方式共同变化。\n\nBridgeless 意味着：1) 没有 MessageQueue，JS 和 Native 可以同步调用（前提是通过 JSI）；2) native module 的注册和加载流程改变，从"启动时注册所有"变成"按需加载"；3) 事件流程简化，某些需要 bridge 中转的交互可以直接进行；4) 调试工具链变化，旧的远程调试方式不再适用。'
    },
    {
      title: 'React Native DevTools',
      body: '新的官方 DevTools 延续"在真实 RN runtime 附近调试"的方向。它减少旧 Remote JS Debugging 带来的环境偏差，对 Hermes、新架构和后续性能工具更重要。\n\nDevTools 的改进包括：1) 更准确的性能度量，直接从 Hermes 或原生 runtime 采集数据；2) React Debugger 集成，可以在真实设备上进行 React Profiler 分析；3) 网络和存储检查工具；4) 减少了环境差异，代码在调试时的行为更接近生产。'
    },
    {
      title: 'Fabric 与 TurboModules',
      body: 'Fabric 是新 renderer，负责 native UI tree 与 React tree 的协作；TurboModules 是新 native module 系统，让模块按需加载并通过更清晰的类型 contract 暴露能力。\n\n两者的关键作用：1) Fabric 通过 Shadow Tree 在 JS thread 做 layout，再批量 commit 给 main thread，降低主线程压力；2) Fabric 支持 React Concurrent 特性，比如 Suspense、transition；3) TurboModules 通过 TypeScript/Flow 定义接口，编译时生成胶水代码，减少运行时开销；4) TurboModules 按需加载，只有调用时才被实例化，减少启动成本。'
    }
  ],
  deepDive: [
    {
