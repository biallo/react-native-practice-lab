export const lesson = {
  ...
  theme: '跨端 UI',
  whyItMatters:
    'React Native 的核心价值不是"用 JavaScript 写一个移动网页"，而是把 React 的声明式 UI、组件组合和状态驱动模型接到平台原生视图上。\n学习这个阶段要先建立正确概念：JS 负责描述界面和业务状态，iOS/Android 负责真实控件、触摸、滚动、文字排版和平台能力。\n理解这条边界，后面学习性能、新架构、原生模块和升级问题时才不会把 RN 误解成浏览器环境。\nReact Native 的出现解决了移动开发的一个核心痛点：开发者希望复用 Web 生态的开发模式（声明式 UI、热更新、快速迭代），但又不想用 WebView 因为这会损失原生性能。\nRN 的设计思路是在 JS 和 Native 之间建立清晰的通信边界，让 JS 仅负责业务逻辑和 UI 描述，而把渲染和性能关键路径交给 Native。这个架构设计对后续所有版本的优化都有指导意义。',
  features: [
    {
      title: '原生视图渲染',
      body: 'View、Text、Image 不是 div、span、img，而是 React Native 对平台视图能力的抽象。\nReact 负责决定组件树长什么样，RN renderer 再把它映射到 native view hierarchy。\n布局、触摸反馈、滚动惯性和文本渲染因此可以使用系统能力。\n\n这意味着：\n1) CSS 没有 cascade，只有 Flexbox 布局；\n2) 样式来自 StyleSheet 定义的集合，不是任意 CSS 属性；\n3) 没有 DOM API，改用 React Ref 和组件 props 通信；\n4) 触摸事件不同于浏览器（onResponderGrant vs onMouseDown）；\n5) 列表滚动由原生控件处理，性能接近原生应用。这种设计让 RN 应用的流畅度能接近真正的原生应用。'
    },
    {
      title: 'Learn once, write anywhere',
      body: 'RN 不是承诺"一份代码完全无差异跑所有平台"，而是复用 React 的开发模型，同时允许你在必要时为 iOS/Android 写平台差异。\n真正成熟的 RN 项目通常共享业务和大部分 UI，同时保留 Platform 分支、平台文件和原生能力入口。\n\n实践中这通常意味着：\n1) 业务逻辑、数据处理、状态管理 100% 共享；\n2) 通用 UI 组件共享，平台特异性强的组件（如输入法、日期选择）做平台版本；\n3) 原生能力封装成统一 JS API，但可能因平台不同有不同实现；\n4) 导航框架通常需要平台特化（iOS UIViewController vs Android Activity）；\n5) 性能优化策略也因平台而异。这是一种务实的跨端方案，既避免了代码重复，又保留了对每个平台的优化空间。'
    },
    {
      title: 'Bridge 通信模型',
      body: '早期架构中，JS 与 Native 通过异步 Bridge 传递序列化消息。\n这个模型让 RN 可以跨语言工作，但频繁跨边界调用、传递大量数据或依赖同步返回值时会暴露性能和架构限制。\n后来的 JSI、TurboModules、Fabric 都是在改进这条边界。\n\nBridge 架构的关键问题包括：\n1) 所有通信都是异步的，JS 调用原生方法需要排队等待回调；\n2) 消息需要序列化（JSON），大数据量传输低效；\n3) 无法实现高频率的同步调用（如响应触摸事件的精细控制）；\n4) Bridge 队列堆积会导致 JS 线程和 Native 线程同时阻塞；\n5) 调试困难，因为跨越了语言和进程边界。理解 Bridge 的瓶颈对后续学习新架构、性能优化和原生模块集成至关重要。'
    },
    {
      title: '移动端约束不同于 Web',
      body: '移动应用有启动时间、内存、低端设备、离线、系统权限、App Store 发布和原生崩溃等约束。\nRN 学习不能只学组件写法，还要理解打包、原生工程、调试和性能测量这些移动端工程问题。\n\n具体约束包括：\n1) 启动时间：用户期望应用在 1-3 秒内显示首屏，超过则容易被卸载；\n2) 内存：移动设备内存远小于服务器，过多的 JS bundle 和运行时内存会触发系统杀进程；\n3) 低端设备：必须在搭载 2-3 年前 CPU 和 2GB 内存的设备上流畅运行；\n4) 离线优先：网络不稳定是常态，需要本地缓存和离线容错；\n5) 系统权限：摄像头、地理位置等权限需要用户明确授予且随时可撤销；\n6) App Store 发布：iOS 有 App Store Review，Android 有用户权限和恶意软件检查；\n7) 原生崩溃：JS 异常可以被捕获，但原生层崩溃会直接导致应用退出。这些约束往往决定了应用的质量和用户体验。'
    }
  ],
  deepDive: [
    {
